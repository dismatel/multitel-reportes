...
const TENANT_ID = extra.azureTenantId ?? '';
const CLIENT_ID = extra.azureClientId ?? '';
const ALLOWED_DOMAIN = '@grupomultitel.com';
const APP_VERSION = '1.0.0';

const msalInstance = new MSALClient({
  auth: {
    clientId: CLIENT_ID,
...
const LoginScreen: React.FC = () => {
  ...
  const handleLogin = useCallback(async () => {
    ...
    try {
      ...
      const expiry = (Date.now() + 3600_000).toString();
...
const styles = StyleSheet.create({
  ...
  header: {
    backgroundColor: '#3A3A3A', height: 56,
  ...
  securityBadge: {
    ...
    borderWidth: 1, borderColor: '#3A7D1A', gap: 6,
  },
